  ...
  useEffect(() => {
    ...
    const vizInit = () => {
      ...
      function play() {
        ...
        const lambertMaterial = new THREE.MeshLambertMaterial({ color: 0xff00ee, wireframe: true });
        ...
        function makeRoughGround(mesh: THREE.Mesh, distortionFr: number) {
          mesh.geometry.vertices.forEach(function (vertex: any, i: any) {
            ...
            const time = Date.now();
            const distance =
              (SimplexNoise.noise2d(vertex.x + time * 0.0003, vertex.y + time * 0.0001) + 0) *
              distortionFr *
              amp;
    ...
    return (
      <div>
        <Card style={{ maxWidth: 720, width: '100%', margin: '3rem', background: '#1c1c1e' }}>
